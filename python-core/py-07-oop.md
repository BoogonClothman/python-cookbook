...
> **本章关系**：前八节建立了 Python OOP 的完整机制——类与实例、方法绑定、四大特性、属性协议、特殊方法、现代工具箱。本节把这些机制组织成**可复用的设计配方**：面对反复出现的设计问题，前人总结出的成熟解法。本节是**精简版**，只讲原则与 4 个与 Python 关联最强的模式；创建型/结构型/行为型全套 + "模式消亡"对照表 + 反模式实战见配套文件 [`py-07-design-patterns.md`](py-07-design-patterns.md)。
...
本节只挑了与 Python 关联最强的 4 个。**全套设计模式**——创建型（建造者）、结构型（装饰器模式与 `@decorator` 的辨析、代理、外观）、行为型（模板方法、观察者、状态、命令）——加上"模式消亡"对照表与反模式实战案例，见配套文件 [`py-07-design-patterns.md`](py-07-design-patterns.md)。建议顺序：先吃透 7.9.1 的 SOLID 原则，再去配套文件系统过一遍，会快很多。
...
> **衔接预告**：第 8 章"异常处理与上下文管理器"将完整展开 7.7.7 的 `with` 协议与 `__exit__` 的异常语义——这也正是 7.7.2 强调的"资源清理交给 `with` 而非 `__del__`"；第 12 章"元编程"会把 7.2.6 与 7.8.1 伏笔的描述符、类装饰器、元类收拢成体系。设计模式完整版见配套文件 `py-07-design-patterns.md`。
