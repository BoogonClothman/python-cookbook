...
> **学习目标**：建立三个心智模型——"类也是对象"、"方法是绑定的函数"、"封装是约定而非强制"；理解三大底层机制——属性查找链、MRO 线性化、描述符绑定；掌握四大特性（抽象、封装、继承、多态）及其争议；能用特殊方法（协议）让自定义类型融入语言语法，并理解对象从 `__new__` 到 `__del__` 的完整生命周期；最后建立设计模式的基本思想（SOLID 原则、Python 特性对模式的替代）。
...
...     def __len__(self): return len(self._items)    # 顺便实现容器协议（7.7.5）
```
...
### 7.7.2 对象的完整生命周期：`__new__` → `__init__` → 使用 → `__del__`

7.1.3 讲了实例创建的前两步——分配与初始化。本节把对象的**完整一生**讲完：从 `__new__` 分配、`__init__` 填充、漫长的使用期，到引用计数归零触发 `__del__`、被垃圾回收。理解这一整段，你才真正知道"Python 对象什么时候死、怎么死"——这是 CPython 内存语义的核心，也是很多资源泄漏 bug 的根源。

**生命周期四阶段全景**：

| 阶段 | 钩子 / 机制 | 谁负责 |
|------|------------|--------|
| ① 分配 | `__new__`（隐式静态方法，收 `cls`） | 返回裸实例；默认 `object.__new__` |
| ② 初始化 | `__init__` | 填充状态；返回值必须为 `None` |
| ③ 使用 | 属性访问 / 方法调用 / 协议分发 | 业务代码（7.1.5、7.4.4、本节 7.7 其余部分） |
| ④ 析构与回收 | `__del__` + 引用计数 + 分代循环 GC | CPython 自动 |
...
④ 析构 a
>>> b                      # b 还活着（③ 使用期）
<__main__.Life object at 0x...>
>>> del b                  # 退出前手动清掉
④ 析构 b
```

**`__new__` 是分配器，不是初始化器。** 它在内存里开辟一块裸对象、绑定类型信息，再交给 `__init__`。三个契约要记牢：

1. **第一参数是 `cls`**（要创建的类），虽不写 `@staticmethod`，行为上就是静态方法
2. **返回值必须是实例**；默认由 `object.__new__(cls)` 完成分配
3. **若 `__new__` 返回的不是 `cls`（或其子类）的实例，`__init__` 根本不会被调用**——初始化被整体跳过：

```python
>>> class Skip:
...     def __new__(cls, data):
...         return data                 # 返回了一个无关对象（这里是 int）
...     def __init__(self, data):
...         print("不会执行到这里")
>>> s = Skip(42)
>>> s, type(s)                          # s 是 42，Skip.__init__ 从未被调用
(42, <class 'int'>)
```

正常情况 `__new__` 在返回实例前，已经可以往实例上挂属性（分配完成、字典可用）：

```python
>>> class Probe:
...     def __new__(cls, name):
...         inst = super().__new__(cls) # 分配裸实例
...         inst._born = True           # 分配完即可挂属性——早于 __init__
...         return inst
...     def __init__(self, name):
...         self.name = name
>>> p = Probe("x")
...
> **实战建议**：绝大多数类只需写 `__init__`。需要覆盖 `__new__` 的只有两类场景——**不可变类型**（`tuple`/`str` 子类的内容必须在分配时定死，`__init__` 里改不了）和**单例**（每次返回同一个实例，7.9.2 会用到）。第 12 章元类将揭示 `__new__` 与元类 `__call__` 的分工。

**`__init__` 的两个边界**：返回值必须为 `None`（否则 `TypeError`）；它不做分配，只做状态填充。`Cls(...)` 在类型系统里是"`type.__call__` 依次转发给 `__new__` 与 `__init__`"（7.1.3 已演示）。

**使用期的长度由"还有没有引用"决定。** 对象只要还被某个名字、容器、闭包、回调持有，就活着；最后一个引用消失的那一刻，进入第④阶段。这直接引出本节的另一半：对象怎么死。

**`__del__`：析构钩子——对象临终前的最后一件事。** 当引用计数归零，CPython 在**同一时刻**调用 `__del__`，随后释放内存：
...
`del x` 只是"删掉名字、减去一个引用"。如果还有别的引用（如被列表持有），对象不会死：

```python
>>> x = Life("x")
>>> keep = [x]           # 列表又持有一个引用 → 计数 2
>>> del x                # 2→1，没归零 → 不析构
>>> keep.clear()         # 最后一个引用消失 → 此刻才析构
④ 析构 x
```

`sys.getrefcount(x)` 可以实测引用计数——注意返回值是**实际引用数 + 1**，因为参数本身又临时持有一个引用：

```python
>>> import sys
>>> x = Life("x")
>>> sys.getrefcount(x)   # 至少 2：全局名 x + 函数参数这一个临时引用
2
>>> keep = [x]
>>> sys.getrefcount(x)   # 3：x + keep + 临时
3
>>> del x
④ 析构 x
```

> **⚠️ 陷阱 1——`__del__` 不保证被调用**：只要还有一丝引用（被遗忘的全局、容器、回调闭包、甚至解释器退出时还没清掉的模块级名字），`__del__` 就无限期延后甚至根本不执行。**把资源清理（关文件、断连接）写进 `__del__` 是经典反模式**——它不可靠、难测试、时机不可控。资源清理的正解是上下文管理器（第 8 章），`__del__` 只该是"没人记得关时的兜底"。

> **⚠️ 陷阱 2——`__del__` 里抛异常会被静默吞掉**：

```python
>>> class Exploding:
...     def __del__(self):
...         raise ValueError("boom")     # 异常无处可去
>>> e = Exploding(); del e
Exception ignored while calling deallocator <function Exploding.__del__ ...>
Traceback (most recent call last):
  ...
ValueError: boom
```

异常只打印一句 `Exception ignored`，**不会传播**到任何 `try/except`。Python 3.8 起这类"无处报告的异常"统一交给 `sys.unraisablehook` 处理；报错文案在 3.14 略有变化（旧版是 "Exception ignored in: <function ...>"），机制不变。

> **⚠️ 陷阱 3——循环引用**：两个对象互相持有，引用计数永远到不了零：
...
...         print("Node 析构")
>>> a, b = Node(), Node()
>>> a.other, b.other = b, a   # 互相引用 → 各计数 2（含全局名）
>>> del a, b                  # 2→1，谁也不归零 → __del__ 不触发
```

这类"引用计数救不回来"的循环，必须靠**循环垃圾回收器**。

**垃圾回收：引用计数 + 分代循环 GC。** CPython 用两层回收：

1. **引用计数**——确定、即时。绝大多数对象在引用归零那一刻被立刻回收。这是 CPython 与 JVM/Go 的本质差异：**对象何时死，在 CPython 里几乎确定可知**（JVM 的 GC 时机不可预测）。
2. **分代循环 GC（`gc` 模块）**——专门抓引用计数漏掉的**循环引用**。对象按存活时长分三代（第 0/1/2 代），新对象进第 0 代，挺过一次回收升一代；`gc.collect()` 手动触发全量回收，`gc.set_debug` 可观察回收过程：

```python
>>> import gc
>>> a, b = Node(), Node(); a.other, b.other = b, a
>>> del a, b
>>> gc.set_debug(gc.DEBUG_SAVEALL)    # 把回收对象存进 gc.garbage 供检查
>>> gc.collect()                      # 手动触发：循环 GC 此刻才找到它们
Node 析构
Node 析构
2                                     # 回收了 2 个对象
>>> gc.garbage                        # DEBUG_SAVEALL 下能看到被回收的循环
[<__main__.Node object at 0x...>, <__main__.Node object at 0x...>]
>>> gc.garbage.clear(); gc.set_debug(0)
```

> **版本注意**：Python 3.4 之前，带 `__del__` 的对象若处于循环引用中，将**永远无法回收**——`gc` 收集不了它们，还会堆积在 `gc.garbage`。**PEP 442（Python 3.4）实现"安全终结"**：循环中的 `__del__` 对象也能被收集（先析构后释放），`gc.garbage` 基本只剩"被 `__del__` 复活"的对象。老文章里"循环引用 + `__del__` = 内存泄漏"的说法在 3.4+ 已不成立。
>
> **跨实现注意**：引用计数是 **CPython 的实现细节**，不是 Python 语言规范。PyPy、Jython 用**跟踪式 GC**（时机不可预测、通常更晚），`__del__` 在那里的触发更不靠谱。依赖"销毁时机"的代码（日志、计数、清理）在跨实现时行为会变。

**`weakref`：不增加引用计数的"弱引用"。** 有些场景需要"记住某个对象，但不影响它死活"——缓存、观察者注册表。弱引用正是为此而生：它**不增加引用计数**，referent 正常死亡，弱引用自动变成 `None`：

```python
>>> import weakref
>>> class Cache: pass
>>> c = Cache()
>>> r = weakref.ref(c)    # 建立弱引用
>>> r() is c              # referent 活着时，调用返回它
True
>>> del c                 # 引用计数归零 → 对象死亡
>>> r() is None           # 弱引用自动失效
True
...
`WeakValueDictionary` 是实战最常用的形态——"值不占用强引用"的自动清理缓存：
...
>>> wv["key"] = o         # 存入缓存
>>> del o                 # 唯一的强引用消失
>>> dict(wv)              # 缓存自动清空，无需手动清理
{}
```

> **注意**：实例要能被弱引用，必须支持 `__weakref__`。默认类都有；但**用了 `__slots__` 的类若没把 `"__weakref__"` 写进槽位，就不支持弱引用**（呼应 7.8.3）：

```python
>>> class Slotted:
...     __slots__ = ("x",)          # 没有 __weakref__ 槽位
>>> weakref.ref(Slotted())         # ❌
Traceback (most recent call last):
  ...
TypeError: cannot create weak reference to 'Slotted' object
>>> class SlottedOk:
...     __slots__ = ("x", "__weakref__")   # ✅ 显式声明
>>> weakref.ref(SlottedOk())       # 正常
<weakref at 0x...>
```

> **工程影响：用 `with` 而不是 `__del__` 管资源。** `__del__` 的所有不可靠（不保证调用、异常被吞、循环引用拖延、跨实现差异）指向同一个结论——**确定性的资源生命周期交给上下文管理器（第 8 章）与 `try/finally`**。把"关闭文件"写进 `__del__` 是教科书级反模式：文件描述符要在解释器退出时才可能被回收，长跑程序会一路泄漏到爆。`__del__` 的正确定位是"兜底"，不是"正路"。

### 7.7.3 表示协议：`__repr__`、`__str__`、`__format__`
...
### 7.7.4 数值与比较协议：`__eq__`、`__hash__`、`__bool__`
...
| 特殊方法 | 作用 | 备注 |
|---------|------|------|
| `__len__` | `len(x)`、真值（7.7.4）、`range(len(x))` | 无 `__bool__` 时兼管真值 |
...
> **本章关系**：前八节建立了 Python OOP 的完整机制——类与实例、方法绑定、四大特性、属性协议、特殊方法、现代工具箱。本节把这些机制组织成**可复用的设计配方**：面对反复出现的设计问题，前人总结出的成熟解法。本节是**精简版**，只讲原则与 4 个与 Python 关联最强的模式；创建型/结构型/行为型全套 + "模式消亡"对照表 + 反模式实战见配套文件 [`chapter-07-design-patterns.md`](chapter-07-design-patterns.md)。

### 7.9.1 什么是设计模式：GoF、SOLID 与"模式会消失"

**设计模式（design pattern）** 的概念借自建筑学，1994 年《设计模式》一书（GoF，Gang of Four）把它引入软件工程：**对反复出现的设计问题的、命名过的、可复用的解法**。一个模式有四要素：**名称**（让团队有共同词汇）、**问题**（它解决什么）、**方案**（结构与协作方式）、**后果**（权衡与代价）。

**模式不是算法。** 算法给"确切的步骤"，模式给"设计骨架"——同样一个工厂，可以用 `if/elif`、可以用 `@classmethod`、可以用注册表，骨架相同、血肉各异。

**模式背后的原则。** 先理解原则再读模式，因为模式是原则的具体化。OOP 领域最常引用的是 **SOLID**：
...
| I | 接口隔离（ISP） | 不强迫客户端依赖它用不到的接口 |
| D | 依赖倒置（DIP） | 依赖抽象，不依赖具体实现 |

其中 OCP 已在 7.5.2 以"覆盖是扩展而不修改"的形式出现过，"组合优先于继承"（7.5.6）也是原则级忠告。**读每个模式时都问一句：它把哪条原则落地了？** 答得上来，才算真懂。

**Python 视角的核心论点：语言特性会让模式"消失"。** GoF 写作时的 C++ 里，许多模式是为了补语言的缺——迭代器模式要手写迭代器类，Python 里一个生成器函数搞定；策略模式要建策略类层次，Python 里直接传函数；适配器要写包装类，Python 里鸭子类型天生兼容。因此讲 Python 的设计模式，必须同时回答"**这个模式在 Python 里还需要吗？不需要的话，替代它的语言特性是什么？**"——这正是"模式消亡"对照表（见配套文件）的主题。

### 7.9.2 单例（Singleton）：全局唯一，先问"真的该用吗"

**问题**：全局只需要一个实例（配置、连接池、日志器）。**方案**：把"只产一个实例"的约束固化进类。三种 Python 实现，按推荐度排序：

```python
# 方式一：模块级单例（最 Pythonic，默认选它）
# config.py —— 模块加载时创建一次，import 同一模块天然共享
class Config:
    def __init__(self):
        self.debug = False
config = Config()

# 方式二：__new__ 拦截（7.7.2 刚讲过 __new__ 契约）
class Singleton:
    _instance = None
    def __new__(cls):
...
# 方式三：元类（第 12 章细讲，先给代码留个印象）
class MetaSingle(type):
    def __call__(cls, *a, **kw):
        if not hasattr(cls, "_inst"):
            cls._inst = super().__call__(*a, **kw)
        return cls._inst
class S(metaclass=MetaSingle): ...
```

**"真的该用吗"**——单例在 Python 里常被当成**反模式**看待：它本质是"披着类的全局变量"，隐式依赖让测试很难注入替身，并发下还要加锁。社区主流替代是**模块级实例 + 显式注入**。**结论**：要"唯一实例"时先写模块级单例；只有"这个类必须从构造上保证唯一"（如某些库 API 要求）才用 `__new__`/元类版本。

### 7.9.3 工厂（Factory）：把"造对象"从"用对象"里解耦

**问题**：创建对象的逻辑散落在调用处，想切换实现要改多处。**方案**：把"决定造哪个、怎么造"集中到一个入口。Python 里最常见的两个形态：

**工厂方法——`@classmethod` 备选构造器就是它**（7.2.3 的 `Date.from_iso`）。工厂方法的本质是"**让类自己提供造自己的多种入口**"，且用 `cls(...)` 而非硬编码类名，保证子类也能正确创建。

**简单工厂函数——不需要类层次时，一个函数足够**：

```python
def make_serializer(fmt: str):
    """集中注册，调用处一行搞定——新增格式只改这里"""
    if fmt == "json":
        return JsonSerializer()          # JsonSerializer / XmlSerializer 在别处定义
    if fmt == "xml":
        return XmlSerializer()
    raise ValueError(f"未知格式 {fmt}")
```

**什么时候才该抽象出工厂**：当"造什么对象"会变（多实现可切换、配置驱动），或创建过程复杂（参数组装、缓存）。**反模式**：只有一个实现、一辈子不会变也硬套工厂——那是"为模式而模式"。

### 7.9.4 适配器（Adapter）：接口不同，鸭子来凑

**问题**：现有类的接口和目标接口对不上，又不想改它。**方案**：包一层做"翻译"。GoF 时代要手写适配器类；**Python 里鸭子类型让适配器常常根本不需要**——只要目标函数用到的接口恰好都有，直接传：
...
需要适配时，一行包装即可（组合 + 转发，7.5.6）：

```python
class Adapted(OldReader):
    def read(self):
        return self.readline()      # 补齐目标接口
```

配合 `__getattr__`（7.4.4）还能写"未定义属性全委托"的自动适配器，省掉大量透传方法。**决策**：适配成本小于改源码成本才用适配器；否则直接改接口。

### 7.9.5 策略（Strategy）：行为是参数

**问题**：同一操作有多种算法（排序的不同比较规则），调用方想随时切换。**方案**：把算法封装成可替换的策略。C++/Java 里策略是一组类；**Python 里策略就是函数或 `functools.partial`**——第 6 章"函数是一等公民"直接消灭了这个模式：

```python
def bubble(data): ...
def quick(data): ...

def sort_with(data, strategy):     # 策略作为参数传入
    return strategy(data)

sort_with([3, 1, 2], bubble)       # 传函数即可，不需要策略类
```

内建接口的设计正是如此：`sort(key=..., reverse=...)`、`max(default=...)` 都是"把策略当参数"。**模式的意义在 Python 里被语法消解了——这就是 7.9.1 说的"模式会消失"。**

### 7.9.6 完整模式库：见配套文件

本节只挑了与 Python 关联最强的 4 个。**全套设计模式**——创建型（建造者）、结构型（装饰器模式与 `@decorator` 的辨析、代理、外观）、行为型（模板方法、观察者、状态、命令）——加上"模式消亡"对照表与反模式实战案例，见配套文件 [`chapter-07-design-patterns.md`](chapter-07-design-patterns.md)。建议顺序：先吃透 7.9.1 的 SOLID 原则，再去配套文件系统过一遍，会快很多。
...
| 对象生命周期 | `__new__`→`__init__`→使用→`__del__`；引用计数即时回收 + 分代循环 GC（PEP 442）；`weakref` 不增加引用计数；**资源清理用 `with` 而非 `__del__`** |
| 设计模式 | 模式=命名过的设计经验；SOLID 是地基；**Python 语言特性让模式"消失"**；单例/工厂/适配器/策略见 7.9，完整版见配套文件 |
...
**11.（预测输出）** 下面代码的 `__del__` 输出顺序是什么？解释 `del a` 为什么没有立即销毁 `a`，并据此说明"资源清理不该写进 `__del__`"的理由（提示：7.7.2 的引用计数）。
...
**12.（设计）** 你的项目有三个需求：① 全局唯一的日志配置；② 支付方式（信用卡/支付宝/现金）可扩展；③ 对同一份数据在不同场景用不同排序算法。分别指出 7.9 中对应的模式，并判断在 Python 里"该用语言特性还是显式模式"、为什么。
...
- ✅ 能描述对象从 `__new__` 到 `__del__` 的完整生命周期，并解释引用计数与循环 GC 的分工
- ✅ 能说出 SOLID 五原则的要点，并判断常见场景该用语言特性还是显式模式
- ✅ 能判断何时用 `@dataclass`、`Enum`、`__slots__`

> **衔接预告**：第 8 章"异常处理与上下文管理器"将完整展开 7.7.7 的 `with` 协议与 `__exit__` 的异常语义——这也正是 7.7.2 强调的"资源清理交给 `with` 而非 `__del__`"；第 12 章"元编程"会把 7.2.6 与 7.8.1 伏笔的描述符、类装饰器、元类收拢成体系。设计模式完整版见配套文件 `chapter-07-design-patterns.md`。
