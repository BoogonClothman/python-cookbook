...
**Python 属于第三代高级语言**，但它在设计上大量吸收了第四代语言的声明式思想，让它读起来像"可执行的伪代码"。
...
**Python 的实际情况比"解释型"这个标签更复杂**：CPython 先将源码编译为字节码（`.pyc`），再由虚拟机执行字节码。这是一种"半编译-半解释"的混合模型，事实上接近 Java 的 JVM 架构，但节省了显式编译的步骤。
