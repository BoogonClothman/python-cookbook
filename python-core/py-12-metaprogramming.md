...
> **学习目标**：建立"**代码也是数据**"的元编程心智，掌握装饰器（函数级）、描述符/属性协议（类级）、元类（类创建级）三层能力；理解 `@dataclass`/`@property`/`classmethod` 等方法背后的实现；能够利用元编程写类型校验、插件注册表、声明式 API；掌握元编程的适用范围。

---

第6章我们提到 "函数是一等公民"，第7章我们讲解过"类与对象模型"，本章将这两件事推向极致：**用代码来写代码**。`@property` 为什么能让方法成为属性？`@dataclass` 为什么能自动生成 `__init__`？`classmethod` 的第一个参数为什么是类对象 `cls` ？这些"魔法"不是解释器的特殊照顾，而是**普通 Python 代码实现的普通对象**。读懂它们，你就从"用 Python 编程"升级到"编程 Python 本身"。
