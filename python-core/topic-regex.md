...
| 运算 | 符号 | 定义 | 直觉 |
|------|------|------|------|
| **并**（Union） | $R \cup S$ | $\{w \mid w \in L(R) \text{ 或 } w \in L(S)\}$ | "或者" |
| **连接**（Concatenation） | $R \cdot S$ | $\{xy \mid x \in L(R), y \in L(S)\}$ | "先匹配 R，再匹配 S" |
| **Kleene 星** | $R^*$ | $L^* = \bigcup_{n \ge 0} L^n$（连接运算的闭包） | "重复零次或多次" |
...
**DFA 的判定规则**：给定输入字符串 $w = a_1 a_2 \dots a_n$，DFA 的执行过程：
1. 从 $q_0$ 开始
2. 读取 $a_1$，转移到 $\delta(q_0, a_1)$
3. 读取 $a_2$，转移到 $\delta(q_{\text{step2}}, a_2)$
4. ...依次类推
5. 读完所有字符后，若当前状态 $\in F$，则**接受**；否则**拒绝**
...
**定理 1.2**：正则语言在以下运算下封闭：
- **并**：若 $L_1, L_2$ 是正则，则 $L_1 \cup L_2$ 是正则
- **连接**：若 $L_1, L_2$ 是正则，则 $L_1 \cdot L_2$ 是正则
- **Kleene 星**：若 $L$ 是正则，则 $L^*$ 是正则
- **补**：若 $L$ 是正则，则 $\Sigma^* \setminus L$ 是正则
- **交**：若 $L_1, L_2$ 是正则，则 $L_1 \cap L_2$ 是正则
...
**定理 1.3（泵引理）**：设 $L$ 是正则语言，则存在泵长度 $p \ge 1$，使得对任意字符串 $w \in L$，只要 $|w| \ge p$，就可以写成 $w = xyz$，满足：
1. $|xy| \le p$（泵的部分在前 $p$ 个字符内）
2. $|y| \ge 1$（泵的部分非空）
3. 对任意 $i \ge 0$，$xy^iz \in L$（重复 $y$ 任意次仍在语言中）

**几何直观**：当字符串足够长时，DFA 必然会**重复经过某个状态**（鸽巢原理）。从第一个重复状态到第二个重复状态之间的子串就是 `y`，可以无限重复而不改变接受性。

**经典应用：证明 $L = \{a^n b^n \mid n \ge 0\}$ 不是正则语言**

反证法：假设 $L$ 是正则的，则存在泵长度 $p$。

取字符串 $w = a^p b^p$（$p$ 个 $a$ 后跟 $p$ 个 $b$），显然 $w \in L$ 且 $|w| = 2p \ge p$。

根据泵引理，$w = xyz$，$|xy| \le p$，$|y| \ge 1$。

因为 $|xy| \le p$，所以 $xy$ 完全在前 $p$ 个字符中——即 $xy$ 只包含 $a$，不包含 $b$。因此 $y = a^k$（$k \ge 1$）。

现在考虑 $xy^2z = a^{p+k} b^p$。这个字符串有 $p+k$ 个 $a$ 和 $p$ 个 $b$，显然 $p+k \ne p$，所以 $xy^2z \notin L$。

**矛盾！** 因此 $L$ 不是正则语言。

> **核心结论**：$a^n b^n$ 类语言需要"计数"能力——记住 $a$ 的个数，然后验证 $b$ 的个数相同。有限自动机没有外部记忆，无法计数任意大的 $n$，因此这类语言超出了正则语言的能力范围。

#### 1.4.3 泵引理的其他应用

**示例 1**：证明 $L = \{a^n \mid n \text{ 是质数}\}$ 不是正则语言

假设 $L$ 正则，取泵长度 $p$。取 $w = a^q$，其中 $q$ 是大于 $p$ 的质数。
$w = xyz$，$|xy| \le p$，$y = a^k$（$k \ge 1$）。
考虑 $xy^{q+1}z = a^{q + qk} = a^{q(1+k)}$。
因为 $q \ge 2, k \ge 1$，所以 $q(1+k)$ 是合数（至少有两个大于 1 的因子），故 $a^{q(1+k)} \notin L$。
矛盾。
...
| 解析复杂度 | $O(n)$ | $O(n^3)$（CYK 算法）或 $O(n)$（LL/LR 解析器） |
...
| `R?` | `R\|ε` | 零次或一次 = R 或空 |
| `R{m}` | `R` 重复 m 次 | 固定重复 |
| `R{m,n}` | `$R^m(R|\varepsilon)^{n-m}$`，即 $\bigcup_{k=m}^{n} R^k$ | 区间重复 |
| `[abc]` | `a\|b\|c` | 字符类 = 字符的并 |
...
对于 $n=30$：$2^{29} \approx 5.4 \times 10^8$ 种划分，Python 3.12 实测约 60 秒
对于 n=100：超出任何实际计算能力
```

> **注意**：网上常把这里的复杂度写成 Bell 数 `$B(n)$`，这是不准确的——Bell 数统计的是**集合**的划分（无序、分组无先后），而 `(a+)+` 的回溯对应的是**序列**的有序划分（组合数 $2^{n-1}$）；且 `$B(30)$` 实际约为 $8.5 \times 10^{23}$，与 $10^{14}$ 差着 9 个数量级。
...
|------|------------------------------|--------------------------------------------------|
| 匹配时间 | 最坏 $O(n \cdot m)$，$n=$输入长度，$m=$正则复杂度；可能指数级 | 严格 $O(n)$ |
| 回溯风暴 | 可能发生 | 不可能 |
...
**练习 1**：使用泵引理证明 $L = \{a^n b^n c^n \mid n \ge 0\}$ 不是正则语言。
...
> 定义：$L_1 / L_2 = \{x \mid \exists y \in L_2, xy \in L_1\}$
...
| 主题 | 核心要点 |
|------|---------|
| 形式语言 | 正则语言 $\subset$ 上下文无关语言；泵引理是证明非正则性的工具 |
